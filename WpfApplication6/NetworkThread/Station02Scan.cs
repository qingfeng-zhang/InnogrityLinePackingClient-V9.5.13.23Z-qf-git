...
namespace InnogrityLinePackingClient
{
    partial class NetworkThread
    {
        ...
        public void Set_PLC_Error(int index, int errValue)
        {
            PLCQueryRx[index + 1] = (byte)((errValue & 0xff00) >> 8);
            PLCQueryRx[index] = (byte)errValue;
        }
        public void Set_FL(int index, String FL)
        {
            byte[] FLByte=System.Text.Encoding.Default.GetBytes(FL);

            for (int i=0;i<FLByte.Length;i++)
            {
                PLCQueryRx[index+i] = FLByte[0+i];
            }
  
        }
        public void Set_Event1(int dmTable,int eventId)
        {
            int tmp0 = ((dmTable - 6100) * 2) + 11;  //EVENT ID Calculation

            PLCQueryRxPara[tmp0 + 1] = (byte)((eventId & 0xff00) >> 8);
            PLCQueryRxPara[tmp0] = (byte)eventId;

 

        }
        public void ST2_Error()
        {
            #region Station 2 Error code
